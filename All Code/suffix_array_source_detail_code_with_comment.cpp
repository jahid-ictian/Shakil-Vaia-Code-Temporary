...
int dp[MAX_N][22]; //for lcp of different position
int pos[MAX_N]; //SA te character kothay ache
...
void constructSA() {         // this version can go up to 100000 characters
  ...
  for (k = 1; k < n; k <<= 1) {
      printf("k %d\n",k);      // repeat sorting process log n times
    countingSort(k);  // actually radix sort: sort based on the second item
    /// c(k) te ami sort kore felechi k'th er vithite and c(0) ami abar punoray thik korechi.
...
void computeLCP() {
  ...
  for (i = L = 0; i < n; i++) {             // compute Permuted LCP in O(n)
    ...
    PLCP[i] = L;
    L = max(L-1, 0);    //Current char er jnne 1 minus// L decreased max n times
  }
